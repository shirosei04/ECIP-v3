...
class StudentController extends Controller
{
    ...
    if(Auth::user()->is_verified == "0"){
        ...
        $allFees = StudentAssessment::where('student_id', Auth::user()->id)->sum('running_balance');
            $ifActiveSy = SchoolYear::where('is_current', '1')->first();
                ...
                $selsection = "";
                $track = "";
                return view('student.enrollment', compact('sections', 'scheds', 'ifActiveSy', 'allFees', 'selsection', 'track'));
    }
   ...
   public function selectSection(Request $request){
    ...
    $allFees = StudentAssessment::where('student_id', Auth::user()->id)->sum('running_balance');
    $track = $request->select_track;
    $selsection = $request->select_section;

    if(count($studentCount) != $selectedSection->capacity){
    return view('student.enrollment', compact('scheds', 'sections', 'ifActiveSy', 'selectedSection', 'allFees', 'ifOpenSy', 'track', 'selsection'));
    }else{
   ...
   public function selectSubject(Request $request){
    ...
    //find and return all the selected subjects
        ...
        return view('student.selected-subjects', compact('scheds', 'selectedSection', 'selectedTrack'));
        }
    }
   ...
   public function assessment(){
    ...
    DB::statement("SET SQL_MODE=''");
     $myFees = StudentAssessment::join('school_years', 'school_years.sy_id', '=', 'student_fees.year_id')
                                ->where('student_id', '=', Auth::user()->id)->orderby('created_at', 'desc')->get();
     $allFees = StudentAssessment::where('student_id', Auth::user()->id)->sum('running_balance');
   ...
   public function grades(){
    if(Auth::user()->is_verified == "0"){
        ...
        $selSem = "";
        $studId = "";
        return view('student.grades', compact('grades', 'studId', 'selSem', 'selYear'));
    }
   }
   ...
   public function filtered_grades(Request $request){
    ...
    $selectedSy = SchoolYear::where('sy_id', $request->select_year)->get();    
        ...
        return view('student.grades', compact('mygrades', 'grades', 'selectedSy', 'studId', 'gradelvl', 'selSem', 'selYear'));
   }
   }
