        ...
        <div class="card-body">
            ...
            <div class="card-footer justify-content-center">
                <div class="d-flex mb-3">
                    ...
                    @if($enrollee->status == "1")
                        <a type="button" class="btn btn-primary me-auto p-2" href="{{url('/registryApproval')}}"><span class="btn-label"><i class="fas fa-chevron-left"></i></span> Back to List</a>
                    @else
                        <a type="button" class="btn btn-primary me-auto p-2" href="{{url('/archived-users')}}"><span class="btn-label"><i class="fas fa-chevron-left"></i></span> Back to List</a>
                    @endif
                    @if($enrollee->status == "1")
                        @if(!empty($enrollee->student))
                            <button type="submit" class="btn btn-success p-2 me-2 acceptBtn"><span class="btn-label"><i class="fas fa-check"></i></span> Accept</button>
                        @else
                            <button type="submit" class="btn btn-success p-2 me-2 disabled"><span class="btn-label"><i class="fas fa-check"></i></span> Accept</button>
                        @endif
                    @endif
                </form>
                    @if($enrollee->status == "1")
                        <a type="button" data-bs-toggle="modal" data-bs-target="#rejectModal{{$enrollee->id}}" class="btn btn-danger p-2 me-2 rejectBtn"><span class="btn-label"><i class="fas fa-times"></i></span> Reject</a>
                        <a href="{{url('archive-user/'.$enrollee->id)}}"  type="button" class="btn btn-warning p-2 me-2 archiveBtn"><span class="btn-label"><i class="fas fa-archive"></i></span> Archive</a>
                        @include('modals.reject')
                    @else
                        <a href="{{url('unarchive-user/'.$enrollee->id)}}"  type="button" class="btn btn-warning p-2 me-2 unarchiveBtn"><span class="btn-label"><i class="fas fa-archive"></i></span> Unarchive</a>
                        <a href="{{url('delete-user/'.$enrollee->id)}}"  type="button" class="btn btn-danger p-2 me-2 deleteBtn"><span class="btn-label"><i class="fas fa-trash"></i></span> Delete</a>
                    @endif
...
<script>
    ...
    $('.acceptBtn').click(function (e) {
            ...
            var delete_id = $(this).closest("div").find('.delete_val').val();
                swal({
                title: "Are you sure?",
                text: "Once accepted, you cannot undo it",
                icon: "warning",
                buttons: true,
                dangerMode: true,
                })
        ...
        var delete_id = $(this).closest("div").find('.delete_val').val();
            ...
            .then((willArchive) => {
                if (willArchive) {
                    ...
                    $.ajax({
                        type: "GET",
                        url: '/view-details/archive-user/'+delete_id,
                        data: data,
                        success: function (response) {
        ...
        var delete_id = $(this).closest("div").find('.delete_val').val();
            ...
            .then((willArchive) => {
                if (willArchive) {
                    ...
                    $.ajax({
                        type: "GET",
                        url: '/view-details/unarchive-user/'+delete_id,
                        data: data,
                        success: function (response) {
                            ...
                            .then((result) => {
                                window.location.href = delete_id;
                            });
                        }
                    });
                } else {
            ...
            var delete_id = $(this).closest("div").find('.delete_val').val();
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        ...
                        $.ajax({
                            type: "DELETE",
                            url: '/view-details/delete-user/'+delete_id,
                            data: data,
                            success: function (response) {
                                ...
                                .then((result) => {
                                    window.location.href = '/archived-users';
                                });
                            }
                        });
                    } else {
